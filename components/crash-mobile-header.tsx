...
export function CrashMobileHeader({
  ...
  return (
    <div className="sm:hidden shrink-0 rounded-lg border border-white/[0.06] bg-[#12141a] px-2 py-1 font-arcade">
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0 flex items-baseline gap-1">
          <span
            className={`text-base font-extrabold tabular-nums leading-none text-amber-300 ${tickClass}`}
            style={{ textShadow: '0 0 12px rgba(251,191,36,0.28)' }}
          >
            {formatBetTokens(blackballsBalance)}
          </span>
          <span className="text-[10px] font-extrabold text-amber-200/90">{CURRENCY_LABEL}</span>
          <span className="text-[9px] font-bold text-white/35 tabular-nums truncate hidden xs:inline">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <div className="text-right leading-none">
            <span className="text-[8px] font-bold text-white/35 uppercase">#{roundId}</span>
            <span className={`ml-1.5 text-sm font-extrabold tabular-nums ${phaseClass}`}>
              {phaseLabel}
            </span>
          </div>
          {isDemoWallet && onDemoRefill ? (
            <button
              ...
              className="touch-manipulation min-h-[24px] px-1.5 py-0.5 text-[9px] font-extrabold rounded-md bg-emerald-500/20 text-emerald-300 border border-emerald-500/40"
            >
              +{DEMO_REFILL_BB}
            </button>
          ) : null}
